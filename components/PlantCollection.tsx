...
const isEqual= (previusProps: PlantEntryType, newProps: PlantEntryType) => {
  return previusProps.plant.plantName === newProps.plant.plantName 
}
    ...
    <Link href={`/entry/${slug}`}>
      <a title={`Go to ${plantName}`}>
        <div className="opacity-95 hover:opacity-100">
          <Image src={image.url} aspectRatio="1:1" width={460} layout="responsive" />
          <div className="p-4">
...
export function PlantEntryVertical({
  ...
  return (
    <div className="opacity-95 hover:opacity-100">
      <Link href={`/entry/${slug}`}>
        <a title={`Go to ${plantName}`}>
          <Image src={image.url} aspectRatio="1:1" layout="responsive" width={624} />
